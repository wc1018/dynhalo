...
def orb_lnlike(
    ...
    out_of_prior = any([p < 0 for p in (r_h, alpha, a)])
    out_of_prior_logd = (-4 > logd) & (logd > 0)
    if out_of_prior or out_of_prior_logd:
        return -np.inf
    ...
    # Compute the negative chi2
    chi2 = -np.dot(d, np.linalg.solve(cov, d))

    # Compute the negative log of the determinant of the covariance. It is
    # highly likely to fall below the numerical precision of `float` since the
    # covariances tend to be very small. To avoid this we scale the covariance
    # by the maximum value squared of the y's. The scaling value is not relevant.
